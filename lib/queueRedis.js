 ...
 * @property {genericPool.Pool} pool Redis connection pool
 */
class RedisQueue extends Queue {
   ...
   * @param {genericPool.Pool} [pool] Redis connection pool. If present used instead of single connection.
   */
  constructor({
    queueName,
    timeout = 30,
    ...
    } else if (pool instanceof genericPool.Pool) {
      debug(`Using provided pool\n${pool}`);
      this.pool = pool;
    } else {
      try {
  ...
  /**
   * Acquire a client from pool and invoke callback on it.
   * @param {function} callback Async funtion to be called. Should have only one argument in witch a redis client will be passed.
   * @returns {void}
   */
  async acquireClient(callback) {
    if (!(this.pool instanceof genericPool.Pool)) {
      throw this._handleError('No pool is available');
    }
    ...
    try {
      ...
      if (this.pool) {
        await this.acquireClient(async client => {
          await client.lpush(this.queueName, prepared);
        });
      } else {
  ...
  async pop() {
    try {
      ...
      if (this.pool) {
        await this.acquireClient(async client => {
          received = await client.brpop(this.queueName, this.timeout);
        });
      } else {
        received = await this.dbClient.brpop(this.queueName, this.timeout);
      }
