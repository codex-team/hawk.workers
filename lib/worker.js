 ...
 * @todo
 *
 * ability to delegate events to another worker?
 *
 *
 *
 ...
 * @property {Map<Object, Promise>} tasksMap - current worker's tasks
...
class Worker {
  ...
  /**
   * Adds ability for delegate tasks via payload to the implementations
   *
   * @return {Error}
   */
  static get delegationType() {
    return new Error(
      'Worker is abstract, please implement to delegate'
    );
  }
  ...
  /**
   * Delegetes task after worker finished
   * @param payload
   * @return {Promise<void>}
   */
  async delegate(payload) {
    await this.channel.sendToQueue(Worker.delegationType, Buffer.from(JSON.stringify(payload)));
  }
  ...
  async start() {
    ...
    const { consumerTag } = await this.channel.consume(this.constructor.type, msg => {
      const promise = this.processMessage(msg);
