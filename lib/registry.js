...
const { createRedisPool } = require('./poolRedis');
 ...
 * @property {Redis} queueConfig.dbClient Database/broker connection client. Passed to Queue to reuse existent connection
 * @property {genericPool.Pool} queueConfig.pool Database/broker connection pool. Passed to Queue to reuse existent connections
...
class Registry {
  ...
  constructor() {
    ...
    if (this.broker === 'redis') {
      // this.queueConfig.dbClient = new Redis(process.env.REDIS_URL); // Uncomment to use single connection instead of pool
      this.queueConfig.pool = createRedisPool(process.env.POOL_MAX);
