...
/**
 * Worker of a single notification channel: consumes the `sender/<channel>` queue
 * and delivers notifications through the channel provider.
 * Created by the multi-channel SenderWorker (src/index.ts).
 */
export default class ChannelSenderWorker extends Worker {
  /**
   * Worker type: queue to consume
   */
  ...
  /**
   * Database Controllers (shared between channels, connections are managed by SenderWorker)
   */
  private eventsDb: DatabaseController;
  private accountsDb: DatabaseController;

  /**
   * @param channelType - channel to serve, defines the queue name
   * @param provider - notifications provider of the channel
   * @param eventsDb - events database controller
   * @param accountsDb - accounts database controller
   */
  constructor(channelType: ChannelType, provider: NotificationsProvider, eventsDb: DatabaseController, accountsDb: DatabaseController) {
    super();
